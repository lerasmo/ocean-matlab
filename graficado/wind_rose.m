...
if 0
    ht(1)=text(-g-ri, 0,'WEST', 'VerticalAlignment','top',   'HorizontalAlignment','left', args{:});
    ht(2)=text( g+ri, 0,'EAST', 'VerticalAlignment','top',   'HorizontalAlignment','right',args{:});
    ht(3)=text( 0,-g-ri,'SOUTH','VerticalAlignment','bottom','HorizontalAlignment','left', args{:});
    ht(4)=text( 0, g+ri,'NORTH','VerticalAlignment','top',   'HorizontalAlignment','left', args{:});
else %errasmo ad hoc cada 45
    ht=[];
    strdir={'Norte','Este','Sur','Oeste'};
    for k=0:45:359
        if k==45
            continue
        end
        ...
        if mod(k,90)==inf
            ht=[ht,text(1.01*g*sind(k),1.01*g*cosd(k),{[num2str(k),'°'],strdir{k/90+1}} ),...
                ];
            
            
        else
            ht=[ht,text(g*sind(k),g*cosd(k),[num2str(k),'°'])];
        end
        if k==180
